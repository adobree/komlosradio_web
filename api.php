...
$streamingType = filter_input(INPUT_GET, 'streamtype');
...
if(!empty($url)) {
	if($streamingType === 'shoutcast') {
		...
		if(!empty($data)) {
			...
			//	$track_list_str = implode("", $track_list); // Az elemeket egyetlen sztringgé fűzzük össze
			//	file_put_contents("player.log", $track_list_str);
			//}
		} else {
			$array = ['error' => 'Failed to fetch data'];
		}
